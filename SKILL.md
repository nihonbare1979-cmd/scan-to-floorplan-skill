...
### A0. 撮影プロトコル(現地・スキャン前にユーザーへ共有する)
歪み(ドリフト)はソフトでは消せない。撮り方で防ぐのが最も効く(実例: 扇町2号は1F壁にじみ22cm=撮り直し推奨級。図面精度の天井がここで決まった)。
1. **ループを閉じる**: 撮影開始地点に必ず戻って終える(Scaniverseが誤差を自己補正できる)
2. **廊下・階段は往復する**(行きだけだと片持ちでズレが蓄積)
3. **部屋の移動は敷居越しに前の部屋を映しながら**ゆっくり(空間のつながりを切らさない)
4. **明るくする**: 全部屋の照明ON・カーテン開ける。暗所はドリフトの主因
5. **ゆっくり歩く・急に振り向かない**。1周5〜10分かけてよい
6. 撮り終えたらその場で `run_pipeline.py` の品質診断にかけ、❌なら**現地にいるうちに撮り直す**

### ★ワンコマンド準備(2026-07導入): A〜Dを一発で
```
cd ~/.claude/skills/scan-to-floorplan/scripts
python3 run_pipeline.py <input.usdz|.glb> <物件名> [--slab=Z] [--yaw=deg] [--flip]
```
projects/<物件名>/ を作り、GLB変換→**品質診断(撮り直し判定)**→姿勢診断(Zヒスト・3方向正投影)→層間の自動検出→統合立面図の先出し→エディタ生成→パラメータ台帳(project.json)まで全自動。
- 終わったら**診断画像5枚を必ずReadで目視確認**(❌撮り直し推奨ならトレースに進まずユーザーへ再スキャン提案)
- 姿勢がおかしければ `--slab= --yaw= --flip` で上書きして再実行(決め打ち禁止の原則は目視確認で担保)
- 続きは E(serve_editorでトレース)へ。以下A〜Dは個別実行したい場合・別撮り時の手順
