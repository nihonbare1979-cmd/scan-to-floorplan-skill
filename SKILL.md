...
## 概要
iPhone等のScaniverseで撮った3Dスキャン(USDZ)を、寸法=点群実測の正確な平面図に起こすパイプライン。築古戸建の1F/2F両方で完成・確立した手順。事実ベースのみ(推定要素は載せない)。
...
### ① USDZ → GLB 変換
ScaniverseでUSDZ書き出し(複数物件で実証済みの形式)。`~/Downloads/`等から拾う。
```
...
1. **3方向の正投影**(生XY/生XZ/生YZ)を個別に高解像度で描き`Read`で目視 → どの面が真上ビュー(間取り)か特定。例: ある物件は1F=Z-up素直, 2F=床が88度倒れていた
2. 床が倒れている場合のみRANSAC床平面で水平化(床法線→Z)。ただし床と壁が同程度の大平面だと乱数で誤検出 → **目視優先**
3. **ヨー(水平面の傾き)**: 占有マップ(2Dヒストグラム)を-15〜15度回転し行/列投影の分散最大の角度を探索。例: 1F=-10.5度, 2F=5.1度
4. **東西/南北反転**: 下から撮影だと鏡像。両軸反転+PIL `FLIP_LEFT_RIGHT`で調整。下から撮ったフロアは反転要
...
- **下敷き描画の使い分け**: 良質で壁が明瞭→腰高スライス(0.9-1.3m引数で指定)+点描 / 壁が薄い・低い→スライス引数を省略して全点+点密度ヒートマップ(壁は床〜天井まで垂直に積もる=高密度を黒く)
...
- **★Chrome連番の罠**: 既存同名があるとChromeは`名前 (1).json`を付ける。「保存した」と言われたら`ls -laht <名前>*.json`で**(1)(2)…と時系列を必ず確認**。中身も配置した部屋数・座標と照合(取り違え事故の実績あり→editor-json-save-pitfall)

### ⑤ 清書
```
python3 generate_floorplan.py <rooms.json> output/<物件名>/<物件>_1f_plan.jpg
```
...
- **階別スキャンの制約**: 1F+2Fが別glbだと点群の垂直連続が無く、ランク(◎○△)と統合立面図(断面)は不可→スキップ。1スキャンに2階分が入っていれば断面も可
- **柱はこれ以上絞ると実測の裏付けが無く"想像"になる**。候補提示で止めるのが誠実(ユーザー方針)

## 出力一式(例)
`output/<物件名>/`: `<物件>_1f_plan.jpg`(平面図) `<物件>_2f_plan.jpg` `<物件>_structural_overlay.jpg`(通し柱)
